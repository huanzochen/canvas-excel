  ...
  const combinedAggSelects = statisticMetrics
    .flatMap((metric) => {
      ...
      if (metric === StatisticsMetrics.Count) {
        ...
        const sumExpr = numericalYs.map((y) => `COALESCE(COUNT("${y}"), 0)`).join(' + ');
        return [`${sumExpr} AS "${metric}"`];
      }

      if (metric === StatisticsMetrics.Sum) {
        const sumExpr = numericalYs.map((y) => `COALESCE(SUM("${y}"), 0)`).join(' + ');
        return [`${sumExpr} AS "${metric}"`];
      }
      ...
      const sumExpr = numericalYs.map((y) => `COALESCE(${sqlFunc}("${y}"), 0)`).join(' + ');
